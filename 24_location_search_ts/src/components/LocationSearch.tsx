...
export default function LocationSearch({ onPlaceClick }: LocationSearchProps) {
  //note that type inferance works with "term" because we are using an empty string so it knows it will be a string
  const [term, setTerm] = useState('');
  const [places, setPlaces] = useState<Place[]>([]);

  // this long type is just what it's called for event
